...
def createBurnData(shp_list:list[Path]):
    row_data = []
    for shp in shp_list:
        csv_fields = ['unique_id','size (acres)','perimiter (miles)','startdateday','startdatemonth','startdateyear','startdateseason','enddateday','enddatemonth','enddateyear','enddatesesason','duration','fire_line (miles)','fire_spread (acres/day)', 'fire_speed (miles/day)', 'dominant_direction','geometry']
        year = str(shp)[-8:-4]
        ...
        filtered_gdf = filtered_gdf.drop(['start_date','end_date'], axis=1)
        print(f'iterating through {year}')
        for _,values in filtered_gdf.iterrows():
            unique_id = f"{year}_{values['fire_ID']}"
            size = values['size'] * 247.185
            perim = values['perimeter'] * 0.6214
            ...
            dur = values['duration']
            fireline = values['fire_line'] * 0.6214
            spread = values['spread'] *247.185
            speed = values['speed'] * 0.6214
